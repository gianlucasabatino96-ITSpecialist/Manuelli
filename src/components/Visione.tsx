...
export function Visione() {
  ...
  const renderCard = (card: VisioneCard, index: number, extraClassName = '') => {
    const isExpanded = expandedCard === card.title

    return (
      <article
        key={card.title}
        className={`group flex w-full flex-col items-start gap-4 rounded-2xl border-t-4 border-verde bg-white p-4 shadow-lg transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-verde/20 md:p-6 ${extraClassName} ${
  ...
  const firstRow = visioneCards.slice(0, 3)
  const secondRow = visioneCards.slice(3)
  ...
  return (
    <section id="visione" aria-labelledby="visione-heading" className="scroll-mt-24 bg-azzurro-bg px-8 py-20">
      ...
      >
        <h2
          id="visione-heading"
          className="section-title text-center text-[1.8rem] font-bold tracking-tight text-azzurro-intenso md:text-[2.5rem]"
        >
          Su cosa lavoreremo insieme
        </h2>
        <p className="section-subtitle mx-auto mt-4 max-w-3xl text-center text-[0.95rem] text-gray-700 md:text-[1rem]">
        ...
        <div className="mt-12 grid grid-cols-1 items-start gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="col-span-1 flex flex-col gap-6 md:col-span-2 md:flex-row md:flex-wrap md:items-start md:justify-center lg:col-span-3 lg:gap-6">
            {secondRow.map((card, i) =>
              renderCard(card, i + 3, 'md:w-[calc(50%-12px)] md:max-w-sm lg:max-w-sm'),
            )}
          </div>
        </div>
      </div>
